...
{
    public class AdjustProductFormController
    {
        private IAdjustProductForm _form;
        ...
        public AdjustProductFormController(IAdjustProductForm form)
        {
            _form = form;
        ...
        public void DataGridView1_EditingControlShowing(object sender, DataGridViewEditingControlShowingEventArgs e)
        {
            ...
            if (_form.GetGridSelectedColumnIndex() == 0)
            {
            ...
            if (_form.GetGridSelectedColumnIndex() >= 1)
            {
                e.Control.KeyPress += CheckKey;
            }
        }
        public void OnBtnAdaugaClick(object sender, EventArgs e)
        {
            ...
            MyConn.Close();
            _form.SetBtnAdaugaEnabled(false);
        }
        public void OnBtnStergeClick(object sender, EventArgs e)
        {
            OleDbConnection MyConn = new OleDbConnection(ConnStr);
            MyConn.Open();
            for (int i = 0; i < this.grdRemoveProducts.RowCount - 1; i++)
            ...
            MyConn.Close();
            _form.SetBtnStergeEnabled(false);
        }
    }
}
